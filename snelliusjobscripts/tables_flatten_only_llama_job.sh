...
#SBATCH --job-name=flatten_llama-final-pls
#SBATCH --output=flatten_llama_final%j.out
#SBATCH --error=flatten_llama_final%j.err
...
CHECKPOINT_HOME="$HOME/table_flatten_only_llama_summary_checkpoint.json"
CHECKPOINT_TMP="$TMPDIR/table_flatten_only_llama_summary_checkpoint.json"
...
python $HOME/tables_flatten_only_llamajob.py "$TMPDIR/observations.json" "$TMPDIR/tables.csv" "$TMPDIR/llama_summaries_out"
...
cp -r "$TMPDIR/llama_summaries_out" $HOME/

...
def extract_column_samples(df, n_samples=3):
    """
    Returns a string representing each column and a few example values,
    in the format: Column: [val1, val2] | Column2: [valA, valB]
    """
    column_summaries = []
    all_unique_row_samples = []

    for col in df.columns:
        sample_values = sample_unique_row_values(df, col, n=n_samples)
        all_unique_row_samples.append(sample_values)
        sample_str = f"{col}: {sample_values}"
        column_summaries.append(sample_str)

    return " | ".join(column_summaries), all_unique_row_samples




def generate_table_summary(model, df, original_metadata_text, generation_params):
    """
    Generate a one pass concise summary of the dataset using column headers and sample values.
    """

    schema_summary_str, all_unique_row_samples = extract_column_samples(df)

    input_text = f"""De dataset bevat de volgende kolombeschrijvingen: {schema_summary_str}
    ...
    outputs = model(messages, **generation_params)
    generated_table_summary = outputs[0]["generated_text"][-1]["content"]

    return generated_table_summary, schema_summary_str, all_unique_row_samples
...
def llm_summarize_all_tables_with_checkpoint(model, table_data_dfs, tables_metadata_df, summary_dict, checkpoint_path):
    ...
    # top_k + repetition_penalty + eos_token_id + num_return_sequence to avoid near infinite reprtition, building on knowledge of model
    column_generation_params = {
        "max_new_tokens": 512,
        "temperature": 0.6,
        "top_p": 0.9,
    ...
    for table_id, df in table_data_dfs.items():
        ...
        generated_table_summary, schema_summary_str, all_unique_row_samples = generate_table_summary(model, df, original_metadata_text, table_generation_params)

        elapsed_ms = (time.time() - start_table_time) * 1000
        print(f"Elapsed time (ms): {elapsed_ms:.0f}")

        # Collect summary dict for this table
        summary = {
            "table_id": table_id,
            "original_metadata_text": original_metadata_text,
            "table_w_sample_summary": generated_table_summary,
            "schema_summary": schema_summary_str,
            "sampled_values": all_unique_row_samples
        }
        ...
        if processed_count % 50 == 0:
            ...
            home_checkpoint_path = os.path.expanduser("~/table_w_samples_llama_summary_checkpoint.json")
...
if __name__ == "__main__":
    ...
    checkpoint_path = os.path.expanduser("~/table_w_samples_llama_summary_checkpoint.json")
   ...
   # Timing the generation job
    ...
    print(f"\n✅ Done! Total time elapsed: {minutes}m {seconds}s {milliseconds}ms")

    
    # Get the TMPDIR path from environment variable
    tmpdir = os.environ.get("TMPDIR", ".")
    save_path = os.path.join(tmpdir, "final_table_w_samples_llama_summary_output.json")
    ...
    # Copy checkpoint file from scratch to home directory
    home_checkpoint_path = os.path.expanduser("~/final_table_w_samples_llama_summary_output.json")
    shutil.copy(checkpoint_path, home_checkpoint_path)
    print(f"Checkpoint copied to home at {home_checkpoint_path}")

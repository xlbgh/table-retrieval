...
def extract_columns(df):
    """
    Returns a string representing each column
    in the format: Column1 | Column2
    """
    column_summaries = []

    for col in df.columns:
        column_summaries.append(f"{col}")

    return " | ".join(column_summaries)
...
def generate_table_summary(model, df, original_metadata_text, generation_params):
    ...
    schema_summary_str = extract_columns(df)
    ...
    return generated_table_summary, schema_summary_str
...
def llm_summarize_all_tables_with_checkpoint(model, table_data_dfs, tables_metadata_df, summary_dict, checkpoint_path):
    ...
    for table_id, df in table_data_dfs.items():
        ...
        generated_table_summary, schema_summary_str = generate_table_summary(model, df, original_metadata_text, table_generation_params)
        ...
        summary = {
            ...
            "table_flatten_only_summary": generated_table_summary,
        ...
        if processed_count % 50 == 0:
            ...
            home_checkpoint_path = os.path.expanduser("~/table_flatten_only_llama_summary_checkpoint.json")
...
if __name__ == "__main__":
    ...
    checkpoint_path = os.path.expanduser("~/table_flatten_only_llama_summary_checkpoint.json")
    ...
    save_path = os.path.join(tmpdir, "final_table_flatten_only_llama_summary_output.json")
    ...
    # Copy checkpoint file from scratch to home directory
    home_checkpoint_path = os.path.expanduser("~/final_table_flatten_only_llama_summary_output.json")
    shutil.copy(checkpoint_path, home_checkpoint_path)
    print(f"Checkpoint copied to home at {home_checkpoint_path}")
